...
func NewFretBoard() *FretBoard {
	...
	b, err := ioutil.ReadFile("./resource/font/wqy-zenhei.ttf")
	if err != nil {
		log.Fatal(err)
	}
	...
	fondata := draw2d.FontData{Name: "wgy-zenhei", Family: draw2d.FontFamilyMono, Style: draw2d.FontStyleNormal}
	draw2d.RegisterFont(
		fondata,
		font,
	)

	fb.gc.SetFontData(fondata)
...
func (fb *FretBoard) DrawFretBoard() {
...
func (fb *FretBoard) SaveScaleDiagram(filename string) {
	draw2dimg.SaveToPngFile(filename, fb.dest)
}
